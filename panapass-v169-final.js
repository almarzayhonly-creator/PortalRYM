  ...
  async function ranking169(v){
    ...
    const draw=()=>{
      ...
      const podium=rows.slice(0,3).map((x,i)=>{
        ...
        return `<div class="v93-pod p${i+1}"><div class="v93-medal">${med[i]}</div><div><div class="v93-pod-name profile-link" data-sup-id="${E(x.supervisora_id||'')}">${E(x.supervisora_nombre||'')}</div><div class="v93-pod-meta">#${rank} · ${E(x.galera||'')}</div></div><div class="v93-pod-value"><b>${value}</b><small>${aux}</small></div></div>`;
      }).join('');
