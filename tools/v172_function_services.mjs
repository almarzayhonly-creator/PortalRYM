...
const config={
  ...
  phase6OpenUnit:{service:'RYM_CONTROL_UNIT_MODAL',mode:'all-around'}
};
...
for(const [target,cfg] of Object.entries(config)){
  let baseDone=cfg.mode!=='first-base',total=0;
  ...
  if(cfg.mode==='first-base'&&!baseDone)throw new Error(target+' base not found');
  console.log(target,'assignments migrated',total);
}
const comp='<script id="rym-v172-core-composition" src="/modules/core/composition.js?v=172-clean"></script>';
...
if(!index.includes('rym-v172-ena-saldo')){if(!index.includes(comp))throw new Error('core composition marker missing');index=index.replace(comp,comp+inserts);fs.writeFileSync(indexPath,index)}
