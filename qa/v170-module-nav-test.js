...
async function waitModule(page,module){
 ...
 for(let i=0;i<60;i++){
  ...
  const hasNav=/Volver al Portal|Dashboard|Operaciones|Unidades|Flota|Ranking|Historial|Reporte ejecutivo/i.test(text);
  if(!loading && hasNav){ready=true;break;}
 }
 return {ready,ms:Date.now()-started,text:text.slice(0,700)};
}
...
async function testOne(browser,mode,viewport,module,buttonRx){
 ...
 try{
  ...
  if(!b){
   const controls=await page.locator('button,a,[role="button"]').evaluateAll(es=>es.filter(e=>{const r=e.getBoundingClientRect();return r.width&&r.height}).map(e=>({tag:e.tagName,text:(e.innerText||e.textContent||'').trim().replace(/\s+/g,' ').slice(0,120),cls:String(e.className||'').slice(0,120)})).slice(0,80));
  ...
  const row={mode,module,ready:state.ready,loadMs:totalMs,portalBodyClass,visibleText:state.text,consoleErrors,failedRequests:failed,...d,screenshot:f}; report.push(row); console.log('MODULE_QA '+JSON.stringify(row));
...
(async()=>{
 ...
 const modules=[['Panapass',/abrir panapass/i],['Revisados',/gestionar revisados/i],['Control de Auto',/ver flota/i],['GPS',/atender .*prioridades|abrir gps|ver gps/i]];
 try{
 ...
 console.log('MODULE_RESULT '+JSON.stringify(report.map(x=>({mode:x.mode,module:x.module,ready:x.ready,loadMs:x.loadMs,portalBodyClass:x.portalBodyClass,bodyClass:x.bodyClass,error:x.error}))));
