...
async function portalReady(page){return await page.locator('body').innerText().then(t=>/Panapass/i.test(t)&&/Revisados/i.test(t)&&/Control de Auto/i.test(t)).catch(()=>false)}
...
async function backToPortal(page){
 if(await portalReady(page))return true;
 const choices=[page.getByRole('button',{name:/volver al portal/i}),page.getByText(/^\s*Volver al Portal\s*$/i),page.getByRole('button',{name:/inicio/i})];
 for(const loc of choices){const b=await visible(loc);if(b){await b.click().catch(()=>{});for(let i=0;i<20;i++){await page.waitForTimeout(500);if(await portalReady(page))return true;}}}
 return false;
}
async function access(page,mode,module,rx){
 if(!(await backToPortal(page)))throw new Error(`Could not return to portal before ${module} (${mode})`);
 await page.waitForTimeout(500);
 const b=await visible(page.getByRole('button',{name:rx}));
 if(!b){report.push({mode,module,error:'access button not found'});console.log(`MODULE_MISSING ${mode} ${module}`);return false}
 await b.click(); await page.waitForTimeout(1800); await snap(page,mode,module); return true;
}
...
(async()=>{const browser=await chromium.launch({headless:true});try{await run(browser,'desktop',{width:1440,height:1000});await run(browser,'mobile',{width:390,height:844});}finally{await browser.close()}fs.writeFileSync(path.join(out,'module-report.json'),JSON.stringify(report,null,2));console.log('MODULE_RESULT '+JSON.stringify(report));})().catch(e=>{try{fs.writeFileSync(path.join(out,'fatal.txt'),String(e.stack||e))}catch{}console.error(e.stack||e);process.exit(1)});
