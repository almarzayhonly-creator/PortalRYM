...
async function portalReady(page) {
  const labels = ['Panapass','Revisados','Control de Auto','GPS'];
  let found = 0;
  for (const label of labels) {
    const rx = new RegExp(esc(label), 'i');
    const el = await firstVisible(page.getByText(rx));
    if (el) found++;
  }
  return found >= 2;
}
...
async function login(page, mode) {
  ...
  if (!pass) {
    if (await portalReady(page)) {
      results.login[mode] = { ok: true, note: 'Portal already authenticated.' };
      return;
    }
    throw new Error(`No visible password field and portal not ready (${mode})`);
  }

  const userSelectors = [
    'input[type="email"]','input[name*="email" i]','input[name*="user" i]',
    'input[autocomplete="username"]','input[placeholder*="correo" i]',
  ...
  while (Date.now() - started < 35000) {
    ...
    if (await portalReady(page)) {
      results.login[mode] = { ok: true, seconds: Math.round((Date.now()-started)/1000) };
      await page.waitForTimeout(1200);
      await page.screenshot({ path: path.join(outDir, `${mode}-01-after-login.png`), fullPage: true });
      return;
    }
    if (/contraseña\s+incorrecta|credenciales\s+incorrectas|usuario\s+incorrecto|invalid\s+(login|credentials)/i.test(lastText)) {
      throw new Error(`Portal rejected QA credentials (${mode})`);
    }
  }
  await page.screenshot({ path: path.join(outDir, `${mode}-01-login-timeout.png`), fullPage: true }).catch(() => {});
  throw new Error(`Portal did not reach Centro de Control within 35s (${mode}). Visible text: ${lastText.slice(0,700)}`);
}

async function openMenuIfNeeded(page) {
  const menus = [
    page.locator('button[aria-label*="menu" i]'), page.locator('button[title*="menu" i]'),
    page.locator('button').filter({ hasText: /^\s*☰\s*$/ }), page.locator('.hamburger, .menu-toggle, .mobile-menu-btn')
  ];
...
async function clickLabel(page, label) {
  ...
  const candidates = [
    page.getByRole('tab', { name: rx }), page.getByRole('button', { name: rx }),
    page.getByRole('link', { name: rx }), page.locator('[role="button"]').filter({ hasText: rx }), page.getByText(rx)
  ];
...
async function scan(page, mode, name) {
  const metrics = await page.evaluate(() => {
    ...
    for (const sel of sampleSelectors) {
      const el = [...document.querySelectorAll(sel)].find(e => { const r=e.getBoundingClientRect(); return r.width>10&&r.height>10; });
      if (el) {
    ...
    const active=[...document.querySelectorAll('.active,[aria-selected="true"]')].filter(e=>{const r=e.getBoundingClientRect();return r.width>0&&r.height>0;}).slice(0,10).map(e=>{const cs=getComputedStyle(e);return{text:String(e.textContent||'').trim().replace(/\s+/g,' ').slice(0,60),color:cs.color,background:cs.backgroundColor,borderColor:cs.borderColor};});
    return {
      title:document.title, href:location.href,
      viewport:{width:innerWidth,height:innerHeight,scrollWidth:document.documentElement.scrollWidth},
      hasVisualCssLink:!!cssLink, visualCssHref:cssLink?cssLink.href:null,
      ...
      bodyBg:getComputedStyle(document.body).backgroundColor, sample, active,
      horizontalOverflow:document.documentElement.scrollWidth>innerWidth+4, overflow
    };
  });
  ...
  results.screens.push({mode,name,screenshot:filename,metrics,contrast});
...
(async()=>{
  ...
  const md=[
    '# V170 Visual QA',`- URL: ${URL}`,`- Screens tested: ${results.global.screens}`,
    ...
    `- Console errors: ${results.global.consoleErrors}`,`- Failed requests: ${results.global.requestFailures}`,'','## Screens',
