...
let reconstructed = current.replace(
  '<link rel="stylesheet" href="/css/core/production-base.css?v=174">',
  `<style>\n${coreCss}\n</style>`,
);

reconstructed = reconstructed
  .replace(/<link rel="stylesheet"([\s\S]*?) href="\/css\/legacy\/v174\/([^"?]+)\?v=174">/g, (_, attrs, file) =>
    `<style${attrs}>${readFileSync(`css/legacy/v174/${file}`, 'utf8')}</style>`,
  )
  .replace(/<script([\s\S]*?) src="\/modules\/legacy\/v174\/([^"?]+)\?v=174"><\/script>/g, (_, attrs, file) =>
