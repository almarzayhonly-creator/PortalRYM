...
for(const f of css){
  ...
  if(!c.trim()||!/[{][^}]*[}]/s.test(c))fail('CSS vacio o sin reglas: '+f);else ok('CSS contiene reglas '+f);
}
...
if(!loader.includes('sessionStorage'))fail('loader no conserva preview entre login/redireccion');else ok('loader conserva preview entre login/redireccion');
...
if(!boundary.includes("d.querySelector('#view')"))fail('boundary no resuelve #view vivo');else ok('boundary resuelve #view vivo despues del shell legacy');
