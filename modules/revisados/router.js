...
(function(w,d){
  ...
  async function invoke(requested){const route=normalize(requested||'dashboard'),a=app();if(!d.body.classList.contains('v66-revisados'))await a.open();current=routes().includes(route)?route:'dashboard';await a.openTab(current);bind();return current}
