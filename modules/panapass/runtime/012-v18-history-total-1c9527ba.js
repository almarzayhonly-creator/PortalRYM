...
(window.__RYM_PANAPASS_PENDING_AROUND__ ||= []).push(["historial", async function(next,ctx){
const v=ctx.view;
...
async function run(resetPage = true) {
  ...
  try {
    const p = params();
    const [sr, rows] = await Promise.all([rpc('panapass_historial_resumen_v2', p), rpc('panapass_historial_lista_v2', {
      ...p,
      p_page: page,
    ...
    if (next) next.onclick = () => {
      const pages = Math.max(1, Math.ceil(Number(lastSummary?.registros || 0) / pageSize));
      if (page < pages) {
        page++;
        run(false);
      }
    ...
    sbox.innerHTML = '';
    obox.innerHTML = `<div class="alert">${esc(e.message)}</div>`;
  }
}
document.querySelector('#histAll').onclick = () => {
  mode = 'ALL';
  setTabs();
  run(true);
};
document.querySelector('#histCobra').onclick = () => {
  mode = 'COBRA';
  setTabs();
  run(true);
};
document.querySelector('#hb').onclick = () => run(true);
document.querySelector('#hclear').onclick = () => {
  document.querySelector('#hu').value = '';
  document.querySelector('#ho').value = '';
  document.querySelector('#hd').value = minf;
  document.querySelector('#hh').value = maxf;
  run(true);
};
setTabs();
await run(true);
