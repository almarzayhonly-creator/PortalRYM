...
const [scope, units] = await Promise.all([phase3ScopeData(), rpc('panapass_unidades_detalle', {
  p_buscar: null,
  p_limit: 5000
}).catch(() => [])]), um = new Map((units || []).map(x => [norm(x.unidad), x]));
...
document.querySelector('#p3NegCapture').onclick = () => document.body.classList.toggle('capture-mode');
await load();
}]);
