...
const v=ctx.view;
const _v13EmpresaPagosTrabajo=(..._args)=>next();
...
if (capture) tools.insertBefore(field, capture); else tools.appendChild(field);
const sel = field.querySelector('#pmEmpresaFiltro');
const companies = () => [...new Set([...out.querySelectorAll('tbody tr [data-pay-company]')].map(x => String(x.textContent || '').trim()).filter(Boolean))].sort((a, b) => a.localeCompare(b, 'es', {
  sensitivity: 'base'
}));
const fillOptions = () => {
  const current = sel.value;
  const items = companies();
  sel.innerHTML = '<option value="">Todas las empresas</option>' + items.map(x => '<option value="' + esc(x) + '">' + esc(x) + '</option>').join('');
...
const refreshKpis = rows => {
  ...
  [...out.querySelectorAll('.kpi')].forEach(k => {
    ...
    if (!strong) return;
    if (label === 'PENDIENTES CARGADOS') strong.textContent = String(rows.length); else if (label === 'MARCADOS PAGADOS') strong.textContent = String(paid.length); else if (label === 'TOTAL PAGADO') strong.textContent = money(total); else if (label === 'BOLETA') strong.textContent = money(boleta);
  });
};
const apply = () => {
...
if (window.__v36PayFilterObserver) {
  try {
    window.__v36PayFilterObserver.disconnect();
  } catch (_) {}
}
