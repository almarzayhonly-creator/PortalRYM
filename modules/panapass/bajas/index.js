/* Portal RYM Architecture V2 - Panapass Bajas */
(function(w){
  'use strict';
  ...
  function contextFrom(input){
    ...
    if(w.RYM_CONTEXT&&typeof w.RYM_CONTEXT.create==='function')return w.RYM_CONTEXT.create('panapass-bajas');
    return null;
  }
  function requireContext(input){
    const ctx=contextFrom(input);
    if(!ctx)throw new Error('Bajas Panapass: RYM_CONTEXT no disponible');
    if(typeof ctx.api?.panapass?.bajas!=='function')throw new Error('Bajas Panapass: API no disponible');
    return ctx;
  }
  ...
  function canonicalRow(row){
    ...
    return Object.freeze({
      ...
      panapass:text(row.panapass_numero??row.panapass),tags:Object.freeze(tagList),cantidadTags:cantidad,
      saldo,enaConsultadoAt:text(row.ena_consultado_at??row.enaConsultadoAt),
      alertaAdmin:Boolean(row.alerta_admin??row.alertaAdmin),raw:row.raw??row
    });
  }
  ...
  async function load(ctx){
    ...
    const rows=await context.api.panapass.bajas();
  ...
  function filters(rows,opts={}){
    ...
    return (rows||[]).map(canonicalRow).filter(r=>(!g||norm(r.galera)===g)&&(!e||norm(r.empresa)===e)&&(!q||norm([r.unidad,r.placa,r.panapass,r.tags.join(' '),r.empresa,r.galera].join(' ')).includes(q))&&(!st||status(r)===st)&&(!saldo||(saldo==='POSITIVO'?r.saldo>0:saldo==='NEGATIVO'?r.saldo<0:r.saldo===0)));
  }

  function summary(rows){
    const list=(rows||[]).map(canonicalRow),actionable=list.filter(r=>r.cantidadTags>0);
  ...
  function renderRows(host,rows,state){
    ...
    host.querySelectorAll('[data-baja-ena]').forEach(b=>b.onclick=()=>{const row=visible.find(x=>x.unidad===b.dataset.bajaEna);if(!row)return;host.dispatchEvent(new CustomEvent('rym:bajas:ena',{bubbles:true,detail:{row,context:enaContext(row)}}));});
  ...
  async function open(ctx={}){
    ...
    const root=typeof options.target==='string'?document.querySelector(options.target):options.target||context.root||document.querySelector('#view');
    if(!root)throw new Error('Bajas Panapass: contenedor no encontrado');
    root.innerHTML='<div class="card">Cargando bajas Panapass...</div>';
    const rows=options.rows?Object.freeze(options.rows.map(canonicalRow)):await load(context);
