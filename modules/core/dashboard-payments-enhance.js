...
async function enhanceDashboard(){const root=d.querySelector('#phase4GaleraKpis');if(!root||root.dataset.rymPaymentsBusy==='1')return;root.dataset.rymPaymentsBusy='1';try{const data=await home();if(!root.isConnected)return;const p=data?.panapass;if(!p)return;
 const view=root.closest('#view')||root.parentElement||root,mes=monthLabel();const pagado=findText(view,'PAGADO ESTE MES');if(pagado){pagado.textContent='PAGADO EN '+mes.toUpperCase();const card=pagado.parentElement?.parentElement||pagado.parentElement;const muted=card?[...card.querySelectorAll('*')].find(el=>/Monto visible según tu alcance/i.test(el.textContent||'')):null;if(muted)muted.textContent=`Mes actual · desde 01/${String(new Date().getMonth()+1).padStart(2,'0')}/${new Date().getFullYear()}`}
 const rows=Array.isArray(p.galeras_7d)?p.galeras_7d:[];for(const g of GALS){const title=findText(root,g);const card=closestCard(title);const r=rows.find(x=>norm(x.galera)===g)||{registros_7d:0,monto_7d:0};setLabelValue(card,'PAGADAS HOY','PAGOS 7 DÍAS',String(r.registros_7d||0));setLabelValue(card,'MONTO PAGADO','PAGADO 7 DÍAS',money(r.monto_7d||0))}}finally{if(root.isConnected)root.dataset.rymPaymentsBusy='0'}}
